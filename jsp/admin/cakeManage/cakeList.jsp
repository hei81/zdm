...
<body>
	...
	<div class="container-fulid content">
		<div class="funbtn">
			...
			<a class="btn btn-info" href="jsp/admin/CakeManageServlet?action=addReq">增加商品</a>
			<div class="search col-md-4">
						<div class="input-group">
							<form action="jsp/admin/CakeManageServlet" method="get">
								<input type="hidden" value="seach" name="action">
		     	 				<input style="float: left;width: 160px;" type="text" class="form-control" name="cakename" placeholder="输入要搜索的商品名称...">
		       					&nbsp;&nbsp;&nbsp;
		...
		<table class="table table-striped table-hover">
			...
			<c:choose>
				<c:when test="${!empty cakeList}">
					<c:forEach items="${cakeList}" var="i" varStatus="n">
						<tr>
							<td class="noClick"><input type="checkbox" name="choice" value="${i.cakeId}">
							<td>${i.cakeName}</td>
							<td>${i.catalog.catalogName}</td>
							
							<td>${i.price}</td>
							<td class="noClick">
								<a class="btn btn-default btn-xs" href="jsp/admin/CakeManageServlet?action=detail&id=${i.cakeId}">详情</a>
								<a class="btn btn-info btn-xs" href="jsp/admin/CakeManageServlet?action=edit&id=${i.cakeId}">修改</a>
								<a class="btn btn-danger btn-xs" href="jsp/admin/CakeManageServlet?action=del&id=${i.cakeId}" onclick="javascript:return confirm('确定要删除吗？');">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
...
<script type="text/javascript">
	...
	$(".homePage").click(function(){
		window.location="${bsePath}jsp/admin/CakeManageServlet?action=list&page=1";
	})
	$(".prePage").click(function(){
		window.location="${basePath}jsp/admin/CakeManageServlet?action=list&page=${pageBean.prePage}";
	})
	$(".nextPage").click(function(){
		
		window.location="${basePath}jsp/admin/CakeManageServlet?action=list&page=${pageBean.nextPage}";
	})
	$(".lastPage").click(function(){
		
		window.location="${basePath}jsp/admin/CakeManageServlet?action=list&page=${pageBean.pageCount}";
	})
	//控制页面跳转范围
	$(".page-go").click(function(){
		...
		if(page > pageCount || page < 1 ){
			...
			window.location="${basePath}jsp/admin/CakeManageServlet?action=list&page="+page;
		}
	})
	...
	$("#batDel").click(function(){
		...
		if(confirm("你确定要删除"+choices.length+"条用户吗？")){
			...
			window.location = "${basePath}jsp/admin/CakeManageServlet?action=batDel&ids="+ids;
